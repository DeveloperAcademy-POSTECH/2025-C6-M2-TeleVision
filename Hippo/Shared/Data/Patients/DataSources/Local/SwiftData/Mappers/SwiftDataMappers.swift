...
extension SDOperation {
  ...
  static func fromDomain(_ o: Operation, owner patient: SDPatient, in ctx: ModelContext) -> SDOperation {
...
extension SDOperationAsset {
  ...
  static func fromDomain(_ a: OperationAsset, owner op: SDOperation) -> SDOperationAsset {
    let sdAsset = SDOperationAsset(
      id: a.id,
      name: a.name,
      fileExtensionRaw: a.fileExtension.rawValue,
      fileURLString: a.fileURL.absoluteString
    )
...
extension OperationAsset {
  ...
  static func fromSwiftData(_ s: SDOperationAsset) -> OperationAsset {
    OperationAsset(
      id: s.id,
      name: s.name,
      fileExtension: OperationAssetExtension(rawValue: s.fileExtensionRaw) ?? .others,
